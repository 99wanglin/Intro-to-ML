...
# datadict2 = unpickle('./cifar-10-python/cifar-10-batches-py/data_batch_2')
# datadict3 = unpickle('./cifar-10-python/cifar-10-batches-py/data_batch_3')
# datadict4 = unpickle('./cifar-10-python/cifar-10-batches-py/data_batch_4')
# datadict5 = unpickle('./cifar-10-python/cifar-10-batches-py/data_batch_5')
# datadict = unpickle('/cifar-10-python/cifar-10-batches-py/test_batch')

# all_images = datadict['data'] + datadict2['data'] + datadict3['data'] + datadict4['data'] + datadict5['data']
# all_images_label = datadict['labels'] + datadict2['labels'] + datadict3['labels'] + datadict4['labels'] + datadict5['labels']
X = datadict["data"]
Y = datadict["labels"]
...
def cifar10_color(X, num_of_images):
    return resize(X, (num_of_images, 3))
    # outputs Xp
...
def cifar10_classifier_naivebayes(x, mu, sigma, p):
    resized = resize(x, (1,3))
    check = float('-inf')
    for i in range(10):
        value = np.prod(norm.pdf(resized, loc=mu[i], scale=sigma[i]), axis=1)
        value *= p[i]
        if value > check:
            check = value
            output = i
...
for i in range(test_num):
    ...
    # For question 2
    result[i] = cifar10_classifier_bayes(test_X[i], mu, sigma, p)
# print(result)
